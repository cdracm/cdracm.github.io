...
# First, 
I had to convince myself the bug was there. That took an indecent amount of time.
...
told a story that sometimes, when the user typed into the IDE editor, the comments in the code were highlighted not as grayed-out text, but as the code that was commented out.

E.g., this is a comment as it should be, boring grey:
![](/assets/hard/comments_correct.png)

But sometimes, they say, this horror happened instead. Some colors leaked inside where they should not:
![](/assets/hard/comments_incorrect.png)

Of course, the real programmers already know the most probable explanations of this bug, namely:
...
Unfortunately, one unlucky evening I did witness this bug myself, which meant it was real. 
So I had to skip to the fifth acceptance stage instantly.

I had to fix it. So

# Then,
I needed a way to reproduce it.
But it was not reproducible.

However long I furiously typed in the IDE, there was not a single issue.
...
And it also seemed to happen after some significant idle time. 
Usually, when I typed, and typed, and typed some more, the bug never did trigger.
...
my left hand was hmm... 

occupied with the other... activity... 
...
# At last, 
I still needed a coffee and a day to decipher what happened, but it boiled down to this.
...
- And then every PSI element links to a list of highlighters generated while this PSI element was visited.

![diagram](/assets/hard/diagram.svg)
...
Turned out, the only problem was that when the `PSI file` disappears, everything it softly referenced disappears immediately too - including the entire map from PSI elements to their associated highlighters -
...
1. I should have hated PSI more. It's changed, disposed, replaced, garbage-collected and resurrected too randomly.
1. I should have feared soft/weak references more. They are too unpredictable in their timing/threading/guarantees behaviour.
1. I should have been more cautious with this non-conventional thing called NI (Natural Intelligence).
  I tried writing the code using only biologically-based squishy slimy brain cells instead of letting an LLM do it for me, like all normal people do.
